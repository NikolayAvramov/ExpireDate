...
const Header = () => {
  const { onLogout, isAuthenticated, authStatus, setAuthStatus } = useAuth();
  const { basket } = useContent();
  useEffect(() => {
    // Update the authStatus from the cookies when the component mounts
          ...
          {authStatus.authenticated ? (
            <li className="nav-item text-white text-lg">
              <Link
                ...
                onClick={() => {
                  onLogout();
                  setAuthStatus({ authenticated: false, type: null }); // Update authStatus after logout
                }}
              >
                Logout
              </Link>
